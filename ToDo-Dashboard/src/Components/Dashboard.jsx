import React, { useEffect, useState } from 'react';
import './Dashboard.css'; // Import the CSS file for styling

const Dashboard = () => {
  const[name,setName]=useState();
  
  const [tasks, setTasks] = useState([
    { text: 'Learning Programming by 12PM', completed: false },
    { text: 'Learn how to cook by 1PM', completed: false },
    { text: 'Learn how to play at 2PM', completed: false },
    { text: 'Have lunch at 4PM', completed: false },
    { text: 'Going to travel 6PM', completed: false },
  ]);

  const toggleTask = (index) => {
    setTasks((prvtask)=>
      tasks.map((task,id)=>{
        if(id==index){
          return {...task,completed:!task.completed}
        }
        else{
          return task;
        }
      })
    )
  };

  const addTask = () => {
   const newtask=prompt("Enter New Task");
   if(newtask){
    setTasks([...tasks,{text:newtask,completed:false}])
   }
  };
  
  const localtime= new Date().toLocaleTimeString();
  const[time,setTime]=useState(localtime);

  const update=()=>{
    const updatetime=new Date().toLocaleTimeString()
    setTime(updatetime);
  }

  setInterval(()=>{
    update()
  },1000)

  useEffect(() => {
    const userName = prompt("Please enter your name:");
    if (userName) setName(userName);
  }, []);


  return (
    <div className="dashboard">
      <header className="header">
        <img src="data:image/jpeg;base64," alt="Profile" className="profile-image" />
        <h2>Welcome, {name}</h2>
      </header>

      <div className="clock-section">
        <h2>{time}</h2>
      </div>
      <div className="task-list">
        <div className="task-container">
          <div className="task-header">
            <h4>Daily Task</h4>
            <button className="add-button" onClick={addTask}>+</button>
          </div>
          <ul>
            {tasks.map((task, index) => (
              <li key={index} onClick={() => toggleTask(index)} className={task.completed ? 'completed' : ''}>
                <input type="checkbox" checked={task.completed} readOnly />
                <span>{task.text}</span>
              </li>
            ))}
          </ul>
        </div>
      </div>
    </div>
  );
};

export default Dashboard;

